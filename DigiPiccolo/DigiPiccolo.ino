...
Adapted by Aron Steg to talk to the DigiPixel.
http://www.bradsprojects.com/the-digipixel/

Instructions on how to build the hardware (excluding the DigiPixel) and links to the related libraries are on Adafruit's website.
http://learn.adafruit.com/piccolo
...
// LED Latch/Button Shift !load pin, LED/Button clock pin, LED Data Pin, LED Output Enable pin, Buttons data pin)
DigiPixel digiPixel(5,2,6,4,3);  
...
void loop() {
  ...
  digiPixel.drawBox(0, 0, 7, 3, green);
  digiPixel.drawBox(0, 3, 7, 5, yellow);
  digiPixel.drawBox(0, 5, 7, 7, red);
  ...
  for(x=0; x<8; x++) {
    ...
    if(peak[x] <= 0) { // Empty column?
      digiPixel.drawVerticalLine(x, 0, 7, black);  
      continue;
    } else if(c < 8) { // Partial column?
      digiPixel.drawVerticalLine(x, c, 7, black);  
    }

    // The 'peak' dot color varies, but doesn't necessarily match
    // the three screen regions...yellow has a little extra influence.
    y = peak[x]-1;
    digiPixel.setPixel(x, y, 14);
    // if (y <= 2)      digiPixel.setPixel(x, y, green);
    // else if (y <= 4) digiPixel.setPixel(x, y, yellow);
    // else             digiPixel.setPixel(x, y, red);
  }
